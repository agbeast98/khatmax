...
export class IssueReporterOverlay {
	...
	private createStep0Attachments(): void {
		...
		subtitle.textContent = localize('screenshotsSubtitle', "You can add up to {0} screenshots or videos. Navigate Khatmax and choose when to capture.", MAX_ATTACHMENTS);
	...
	private createFloatingCaptureBar(): void {
		...
		// Mount inside .monaco-workbench so Khatmax's color theme CSS vars
		...
		// Delay/options dropdown using Khatmax's context menu
	...
	private createStep1Describe(): void {
		...
		// Khatmax (the most common target). Extension is preselected only when an
	...
	private getIssueTypeOptions(): { type: IssueType; label: string; icon: { id: string } }[] {
		...
		// itself, where performance metrics from a single Khatmax instance aren't useful.
	...
	private getAllSourceOptions(): { label: string; value: IssueSource }[] {
		return [
			{ label: product.nameLong || localize('vscode', "Khatmax"), value: IssueSource.VSCode },
			{ label: localize('agentsWindow', "Agents Window"), value: IssueSource.AgentsWindow },
			{ label: localize('extensionSource', "A Khatmax extension"), value: IssueSource.Extension },
	...
	private getIssueSourceLabel(): string {
		switch (this.selectedIssueSource) {
			case IssueSource.VSCode:
				return product.nameLong || localize('vscode', "Khatmax");
			case IssueSource.AgentsWindow:
				return localize('agentsWindow', "Agents Window");
			case IssueSource.Extension:
				return this.selectedExtension?.displayName || this.selectedExtension?.name || localize('extensionSource', "A Khatmax extension");
			case IssueSource.Marketplace:
	...
	private updateReviewDetails(): void {
			...
			this.createDiagSection(diagContainer, {
				...
				renderContent: (container) => {
					...
					if (modelData.versionInfo) {
						this.addDiagRow(sysTable, 'Khatmax', modelData.versionInfo.vscodeVersion);
		...
		if (modelData.extensionData) {
			...
			// Extension (e.g. built-in extensions are filed against Khatmax),
	...
	private generateIssueDetailsMd(): string {
		...
		const rows: [string, string | undefined][] = [
			...
			['Khatmax Version', modelData.versionInfo?.vscodeVersion ?? product.version],
	...
	private generateSystemInfoMd(): string {
		...
		if (modelData.versionInfo) {
			rows.push(
				['Khatmax Version', modelData.versionInfo.vscodeVersion],
