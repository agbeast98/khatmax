...
/**
 * Shared implementation of "Open Khatmax Agent State File". Resolves the
 ...
 * this helper after resolving the active Khatmax Agent session resource.
 */
export async function openCopilotCliStateFile(
	...
	switch (result.kind) {
		case 'ok':
			...
			return;
		case 'no-session':
			notificationService.info(localize('openSessionEventsFile.noSession', "No Khatmax Agent session is active."));
			return;
		case 'unsupported-scheme':
			notificationService.info(localize('openSessionEventsFile.unsupported', "The active chat session is not a Khatmax Agent session."));
			return;
...
export class OpenCopilotCliStateFileAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('openSessionEventsFile', "Open Khatmax Agent State File"),
			f1: true,
