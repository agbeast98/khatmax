 ...
 *
 * Steps:
 * 1. Sign In — sessions-style sign-in hero with Khatmax AI, Google, and Apple options
...
export class OnboardingVariationA extends Disposable implements IOnboardingService {
	...
	show(): void {
		...
		this.overlay.setAttribute('aria-label', localize('onboarding.a.aria', "Welcome to Khatmax"));
		...
		if (this.footerLeft) {
			if (this._isLastStep()) {
				...
				if (!this._footerSignInBtn && !this._userSignedIn) {
					...
					this._footerSignInBtn.textContent = localize('onboarding.sessions.signInNudge', "Sign in to use Khatmax AI");
	...
	private _renderSignInStep(container: HTMLElement): void {
		...
		title.textContent = localize('onboarding.signIn.heroTitle', "Welcome to Khatmax");

		const subtitle = append(contentMain, $('p.onboarding-a-signin-subtitle'));
		subtitle.textContent = localize('onboarding.signIn.heroSubtitle', "Sign in to use Khatmax AI.");
		...
		copilotDisclaimer.append(localize('onboarding.signIn.disclaimer.copilotPrefix', ". {0} Khatmax AI may show ", defaultChat.provider.default.name));
	...
	private _renderPersonalizeSubtitle(container: HTMLElement): void {
		...
		container.append(
			...
			localize('onboarding.personalize.tip.suffix', " to access all Khatmax commands."),
		);
	}
	...
	private async _applyKeymap(keymapId: string): Promise<void> {
		...
		if (!keymap?.extensionId) {
			return; // Khatmax default, nothing to install
		}
	...
	private _renderAgentSessionsStep(container: HTMLElement): void {
		...
		this._createFeatureCard(moreGrid, Codicon.rocket,
			...
			localize('onboarding.sessions.runAnywhere.desc', "Run agents locally for interactive work, in the background with Khatmax Agent, or in the cloud with cloud agents that open a pull request your team can review."));

		this._createFeatureCard(moreGrid, Codicon.settingsGear,
			localize('onboarding.sessions.customize', "Customize Your Agents"),
			localize('onboarding.sessions.customize.desc', "Tailor Khatmax AI to your project with custom instructions and agents, skills, reusable prompts, and MCP servers that connect to the tools and context you rely on."));
